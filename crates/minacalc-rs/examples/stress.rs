...
fn main() {
    ...
    println!(
        "=== realistic chart ({} notes, ~5min, 8 NPS) ===",
        chart.len()
    );

    let t = Instant::now();
    let ssr = calc
        .calc_at_rate(&chart, 1.0, 0.93, 4, CalcMode::Ssr)
        .unwrap();
    println!(
        "calc_at_rate  1.0x SSR : overall={:.2}  ({:.2?})",
        ssr.overall,
        t.elapsed()
    );
    ...
    println!(
        "calc_all_rates    MSD  : overall@1.0x={:.2}  ({:.2?})",
        overall_at_1x,
        t.elapsed()
    );
    ...
    println!(
        "\n=== stress chart ({} notes, ~7h, 8 NPS) ===",
        stress.len()
    );

    let t = Instant::now();
    let ssr = calc
        .calc_at_rate(&stress, 1.0, 0.93, 4, CalcMode::Ssr)
        .unwrap();
    println!(
        "calc_at_rate  1.0x SSR : overall={:.2}  ({:.2?})",
        ssr.overall,
